...
class Bottleneck(nn.Module):
    ...
    def __init__(self,
        ...
        self.block1 = Conv2dLayer(in_channels, out_channels, kernel_size = 1, bias = bias, 
                                  activation = 'relu', normalize = 'bn', up = up, down = 1, groups = groups)
        self.block2 = Conv2dLayer(out_channels, out_channels, kernel_size = 3, bias = bias, 
                                  activation = 'relu', normalize = 'bn', up = up, down = down, groups = groups)
        self.block3 = Conv2dLayer(out_channels, out_channels * self.expansion, kernel_size = 1, bias = bias, 
                                  activation = 'linear', normalize = 'bn', up = up, down = 1, groups = groups)

        if self.downsample:
            self.downsampling = Conv2dLayer(in_channels, out_channels * self.expansion, kernel_size = 1, normalize = 'bn', up = up, down = down)
