...
from modules.DSFD_basic_modules import Interpolate, FEM, DistillKL, fem_module, add_extras
from modules.detection import Detect
from modules.priorbox import PriorBox
from model.Bio_ReflectNet_Module import Bio_ReflectNet
...
class DSFD(nn.Module):
    ...
    def forward(self, x, x_light, I, I_light):
        ...
        # 밝은 이미지 통과
        features_light, darklevel_light = self.vgg(x_light, reflectance = True) # 모든 레이어 통과
        x_light = features_light[1] # 우리 모델은 stage 2아웃풋을 reflectance 예측에 사용

        # 어두운 이미지 통과
        features, darklevel = self.vgg(x, reflectance = True)
        x_dark = features[1]

        # extract the shallow features and forward them into the reflectance branch
        # extract dark(light) vector for semi-ORT
        dark_vector = self.ref1(x_dark)
        light_vector = self.ref1(x_light)

        R_dark = self.ref2(dark_vector)
        R_light = self.ref2(light_vector)
        ...
        x_dark_2 = self.vgg(x_dark_2, reflectance = True)[1]
        x_light_2 = self.vgg(x_light_2, reflectance = True)[1]

        # Redecomposition
        dark_vector_2 = self.ref1(x_dark_2)
        light_vector_2 = self.ref1(x_light_2)

        R_dark_2 = self.ref2(dark_vector_2)
        R_light_2 = self.ref2(light_vector_2)
        ...
        # --------------------------------
        # SORT loss from "https://github.com/cuiziteng/ICCV_MAET.git" of https://arxiv.org/abs/2205.03346
        loss_sort = self.config.WEIGHT.SORT * torch.mean(torch.abs(self.ort_func (grads['light_grad'].view(batch_size,-1), grads['dark_grad'].view(batch_size,-1))))+\
        ...
        return output, [R_dark, R_light, R_dark_2, R_light_2], loss_mutual, loss_sort
