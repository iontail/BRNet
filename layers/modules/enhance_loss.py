...
class ImprovedDarklevelLoss(nn.Module):
    def __init__(self, alpha=0.7, beta=0.3):
        super(ImprovedDarklevelLoss, self).__init__()
        self.alpha = alpha  # Smooth L1 비중
        self.beta = beta    # LogCosh 비중
        
    def forward(self, dark, dark_light, dark_gt, dark_light_gt):
        # Smooth L1 Loss 계산
        smooth_l1_loss_dark = F.smooth_l1_loss(dark, dark_gt)
        smooth_l1_loss_dark_light = F.smooth_l1_loss(dark_light)

        # LogCosh Loss 계산
        logcosh_loss_dark = torch.mean(torch.log(torch.cosh(dark - dark_gt)))
        logcosh_loss_dark_light = torch.mean(torch.log(torch.cosh(dark_light - dark_light_gt + 1e-12)))

        loss = self.alpha * (smooth_l1_loss_dark + smooth_l1_loss_dark_light) \
            + self.beta * (logcosh_loss_dark + logcosh_loss_dark_light)

        # 최종 스케일링
        loss = loss * cfg.WEIGHT.DARKLEVEL
