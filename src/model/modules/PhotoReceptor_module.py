...
class Rod_Block(nn.Module):
    ...
    """
    def __init__(self, in_channels, out_channels, reduction=2, normalize = 'bn'):
        ...
        self.conv2 = Conv2dLayer(mid_channels, mid_channels, kernel_size=3, activation = 'relu', normalize = normalize)
        self.conv3 = Conv2dLayer(mid_channels, out_channels, kernel_size=3, activation = 'linear', normalize = normalize)


        if in_channels != out_channels:
            self.downsampling = nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=1, bias=False)
...
class Cone_Block(nn.Module):
    ...
    """
    def __init__(self, in_channels, out_channels, normalize = 'bn'):
        super(Cone_Block, self).__init__()

        self.conv1 = Conv2dLayer(in_channels, out_channels, kernel_size=3, activation = 'relu', normalize = normalize)
...
class Photoreceptor_Block(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, reduction=2):
        ...
        self.rod_path = Rod_Block(in_channels, out_channels,reduction)
        self.cone_path = Cone_Block(in_channels, out_channels)
    ...
    def forward(self, x, darkness_level):
        ...
        # darkness_level -> [B,1,1,1] for broadcast
        darkness_level
...
if __name__ == '__main__':
    ...
    batch = 2
    res = 512
    detector = Photoreceptor_Block(3, 10, reduction = 2, cal_darklevel=True).to(device)
    
    img = torch.randn(batch, 3, res, res).to(device)
    darkness_level = torch.randn(batch).to(device)
